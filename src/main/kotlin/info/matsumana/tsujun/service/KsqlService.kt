...
class KsqlService(private val ksqlServerConfig: KsqlServerConfig) {
    ...
    companion object {
        ...
        private val REGEX_SELECT = Regex("""^SELECT\s+?.*""")
        private val REGEX_QUERIES = Regex("""^(LIST|SHOW)\s+?QUERIES\s+?.*""")
        private val REGEX_STREAMS = Regex("""^(LIST|SHOW)\s+?STREAMS\s+?.*""")
        private val REGEX_TABLES = Regex("""^(LIST|SHOW)\s+?TABLES\s+?.*""")
